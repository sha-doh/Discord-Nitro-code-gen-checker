...
process.on("SIGINT", function () {
  console.log('\nChecked ' +i+' codes!\nFound '+a+' working codes haha, get better codes shithead!')
  process.exit();
});
...
const speed = 20 //Measured in milliseconds
...
request.get({
    ...
    try {
            ...
            if(body.message != "Unknown Gift Code" && body.message != "You are being rate limited." && body.message != "404: Not Found"){
                console.log(colors.green(`[CHECKER]AYYY, Code Found, You lucky MF!: https://discord.gift/${code}`));
                fs.appendFileSync("working.txt", code+'\n', "UTF-8",{'flags': 'a+'});
                i++
                a++
            }
            else {
                console.log(colors.red(`${code} Doesnt work!` + colors.white(`${debug ? `\n[DEBUG] Proxy: ${proxy}\n[DEBUG] Message: ${body.message}` : ''}`)));
                i++
            }
        }
        catch (error) {
